...
- The CPU governor on this machine reads `powersave`. Every sweep records it into the result JSON's metadata for provenance; nothing in the harness checks its value or refuses to run.
...
- The benchmark gate runs on this development machine. Every ratio it reports interleaves both arms within one round, so clock drift cancels between them; absolute times and comparisons across separate runs are not comparable.
...
| `bench/harness.jl` | `record!`, `save_results`, `median`, `single_threaded!`, `governor` |
...
- Produces: `record!(; family, routine, variant, eltype, n, m, s, bench, relerr, extra)`; `save_results(stem) -> String`; `median(v)`; `single_threaded!()`; `governor() -> String`; the global `ROWS::Vector{Dict{String,Any}}`
...
recorder and the JSON writer, and neither records the CPU governor. Both move to the shared one
so every sweep records its provenance the same way.
...
`bench/givensq_compaction.jl` takes the guard line only. Its rows are a per-shape crossover fit
...
writer alone. Give it the line above so its metadata carries the same provenance as the others,
and re-run it in Task 6 so the numbers `docs/src/q_representations.md` quotes come from the same
machine as the rest.
...
prints and writes exactly what it did, now with a `"governor"` entry in its file's metadata.
...
the plots and the benchmarks page; it never runs a benchmark. Every sweep records the host's CPU
governor into its result file's metadata; it does not check the value.
...
**Interfaces:**
- Consumes: `record!`, `single_threaded!`, `save_results`; `UpdatableFactorizations.{UpdatableCholesky, lowrankupdate!, lowrankdowndate!, delete_column!, insert_column!, shift_columns!}`; `LinearAlgebra.{cholesky, lowrankupdate!, lowrankdowndate!}`; `UpdatableCholeskyFactorizations.{updatable_cholesky, add_column!, remove_column!}`
...
**Interfaces:**
- Consumes: `cholesky_cells`, `lu_cells`, `qr_cells`, `single_threaded!`, `save_results`
...
All three sweeps run on this development machine. Each records the host's CPU governor into its
result file's metadata for provenance; nothing checks its value or refuses to run.

- [ ] **Step 1: Instantiate and run the three sweeps**
...
The three result files describe this one machine.
...
julia --project=bench -e '
    ...
    meta = JSON.parsefile("bench/results/updating-$(gethostname()).json")
    rows = meta["rows"]
    println("governor: ", meta["governor"])
...
1. The file records one BLAS thread and holds all 198 rows. Its `governor` entry is printed for
   the record, not checked against a required value.
...
- [ ] **Step 3: Bring the Q representations page onto this run**
...
- [ ] **Step 4: Commit the datapoints**

```bash
git add bench/results docs/src/q_representations.md
git commit -m "Record the updating benchmarks"
```
...
ratios in the preamble match a hand computation from the committed `construction-<host>.json`;
the QR figure's x-axis reads `m` and its table's size column holds the QR row counts; and each
figure's legend lists every variant that appears anywhere in that family, with the colors the
panels drew.
...
`evals = 1`, taken with one BLAS thread. The
